...
预热日只构建到上下文日 `t0` 的 raw 输入前缀；从此以后每个信号日 `t` 只追加新出现的日线 raw 快照。M03 对新增日线逐日归一化，M04 使用其有限滚动尾部，并通过上一信号日的 EMA 状态恢复递推值。首日、中段和末日再用完整前缀逐值复算，结果必须一致：
...
预热上下文从本地共同可用的最早 QQQ/VIX/VIX3M 交易日开始；M05 在首个外部请求日之前按时间顺序回放截至上下文日的历史快照，只保留滞回状态和恢复波动率比较尾部；对外输出从 `2025-01-01` 后的第一个有效交易日开始。首个执行日使用输出窗口前一个信号日的目标仓位，避免人为把首日强行设为现金或满仓。这个运行器不会把“全样本一次性计算出的未来指标”写回过去；未来行只能产生自己的快照，且不能改变前缀复算结果。
...
- `phase_observations.csv`、`phase_metrics.csv`：固定标签的上涨/下跌/震荡阶段审计；
