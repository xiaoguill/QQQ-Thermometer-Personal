...
v12.2 的 150 日均线是数据预热要求，不是策略参数；现有 M04 有限尾部还需要 10 个 EMA 状态承载位置，因此程序需要共同有效起点后的前 160 个 NYSE 交易日来建立因果上下文。首个可用信号日期由程序动态计算并写入 Evidence，预热期不计入策略收益，也不生成调仓目标。如果免费两年窗口不足以完成这段预热，则本次仍失败关闭。
