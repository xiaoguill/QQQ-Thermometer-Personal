...
目标：从显式数据源获取 QQQ、BIL、VXX、VIX、VIX3M，裁剪到完整收盘日，并调用既有 v12.2 因果回放生成最新纸上目标。

禁止：在 M19 重新定义策略；把 VIX、SVXY 或 BIL 静默当作 VXX；使用未收盘日线；把 key 写入代码、日志或 Artifact；接入券商。

测试：Massive/local CSV 双模式、VXX 缺失失败关闭、数据截止日、无前视检查、下一交易日执行、源 hash 和离线回放。

验收门槛：`decision.json` 明确标出运行版本、回放版本、策略版本、数据源、数据截止日、数据质量和纸上边界。

### M20 — GitHub Action 三节点与次日白话邮件

目标：在纽约开盘/午盘/收盘运行只读检查，并在北京时间 08:00–10:00 内发送上一收盘的中文摘要邮件。

禁止：使用盘中数据确认收盘目标；把失败结果当成有效仓位；在邮件或日志中暴露 key；创建订单或自动交易；修改旧的独立验证工作流。

测试：工作流 YAML、手动 close、测试邮件、失败 Artifact、邮件服务失败可见性、周六发送周五收盘结果、Secrets 边界。

验收门槛：用户先手动测试再开启定时；每次 Action 保留可下载的运行证据；故障时明确“暂不调仓”。

### M21 — 免费收盘数据优先与单次日收盘运行

目标：在不改变 v12.2/v10 策略规则的前提下，使用 Massive 免费股票日线收盘数据覆盖 QQQ、QLD、VXX、SVXY、BIL、TLT、IAU、XLU、VOO、SPY；使用 Cboe 官方公开 CSV 获取 VIX/VIX3M；每天北京时间早上运行一次，读取上一美股完整收盘。

禁止：把 VIX3M 缺失替换成 VIX、SVXY 或 BIL；把 VXX 的权限、接口和 symbol contract 问题混为一谈；使用盘中或未完成日线；把收费数据源作为隐式依赖；把 M21 当成新的策略版本；接入券商或自动下单。

测试：Cboe CSV 解析与可用性、Massive 股票 symbol contract、VXX 权限/接口/标的定义故障注入、所有关键序列缺失和陈旧、截至收盘日裁剪、v12.2 因果回放、邮件失败关闭、单次 Action YAML 与 Secrets 边界。

验收门槛：`availability_evidence.json` 能逐标的说明来源、日期、hash、状态和 VXX 失败分类；任何必需序列失败时目标为空且邮件写“数据不完整，本次不调仓”；成功时仍明确是纸上目标，不能称为实盘验证。
